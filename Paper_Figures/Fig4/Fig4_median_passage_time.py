...
def plot_median_passage_times(passage_times, size, name, ax, ax2):
    medians, std = {}, {}
    for passage in passages:
        medians[passage] = np.median([time for _, time in passage_times[passage]])
        std[passage] = np.std([time for _, time in passage_times[passage]]) / np.sqrt(len(passage_times[passage]))

    # ax.errorbar([p[:2] for p in passages_forwards], [medians[m] for m in passages_forwards],
    ax.errorbar([pos + 0.5 for pos in range(6)], [medians[m] for m in passages_forwards],
                yerr=[std[m] for m in passages_forwards], capsize=5, label=size + ' forwards',
                color=colors[size])
    # make a twin axis
    ax2.errorbar([pos + 0.5 for pos in range(7)], [np.nan] + [medians[m] for m in passages_backwards],
    ...
    # set xticklabels in Times New Roman
...
if __name__ == '__main__':
    ...
    for size in sizes:
        ...
        plot_median_passage_times(passage_times, size, name, ax, ax2)
    ax.set_ylim(0, 1000)
    ax2.set_ylim(0, 1000)
    ax.set_ylabel('time [s] (forwards)')
    ax2.set_ylabel('time [s] (backwards)')
    ax.set_xlabel('size')
    fig.savefig('Fig4_median_passage_times.png')
    fig.savefig('Fig4_median_passage_times.svg')
    fig.savefig('Fig4_median_passage_times.eps')
