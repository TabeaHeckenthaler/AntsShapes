        ...
        for block_bool in [True, False]:
            for size, dfs in dfss.items():
                for sep, df in dfs.items():
                    ...
                    fig, ax = plt.subplots()
                    create_bar_chart(df, ax, block=block_bool)
...
def plot_sim(block=True):
    fig, axs = plt.subplots(2, sharex='col', figsize=(7, 10),
                            gridspec_kw={'height_ratios': [myDataFrame_sim['winner'].sum(),
                                                           len(myDataFrame_sim['winner']) - myDataFrame_sim['winner'].sum()]})

    create_bar_chart(myDataFrame_sim[myDataFrame_sim['winner']], axs[0], block=block)
    create_bar_chart(myDataFrame_sim[~myDataFrame_sim['winner']], axs[1], block=block)
    plt.subplots_adjust(hspace=.0)
